                        ...
                        <div class="col-12">
                            <div class="series-wrap">
                                ...
                                <div class="section__carousel-wrap">
                                    <div class="section__series flex flex-wrap gap-x-4" id="episodesContainer">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    function loadEpisodes(season, selectedEpisode = 1) {
        $.ajax({
            ...
            success: function(data) {
                ...
                data.episodes['episodes'].forEach((episode, index) => {
                    ...
                    if (episode.episode_number == selectedEpisode) {
                        episodeElement.classList.add('bg-blue-900');
                    }
                    // default
                    if (episode.episode_number !== selectedEpisode) {
                        episodeElement.classList.add('bg-blue-700');
                    }
                    episodeElement.innerHTML =
                        `<button
                        type="button"
                        href="/tv/detail/${tvShowId}?season=${season}&episode=${episode.id}"
                        class="inline-block rounded px-6 pb-2 pt-2.5 text-sm font-medium uppercase leading-normal text-white shadow-primary-3 transition duration-150 ease-in-out hover:bg-primary-accent-300 hover:shadow-primary-2 focus:bg-primary-accent-300 focus:shadow-primary-2 focus:outline-none focus:ring-0 active:bg-primary-600 active:shadow-primary-2 motion-reduce:transition-none   "
                        onclick='handleChangeEpisode(${episode.episode_number},${tvShowId},${season})'
                        id='episode'>
                                     E: ${index+1}
                         </button>`;
